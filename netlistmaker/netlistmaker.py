...
def img_proc(leimg):
    ...
    img_tres = cv2.adaptiveThreshold(
        img_clear, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, BLOCK, C
    )
...
def predict_components(img_skel: np.array, components, model):
    ...
    for i, _ in enumerate(components):
        ...
        square = (
            ...
            x_center - dim,
            x_center + dim,
        )  # expand width to size of height
        ...
        for scale in range(0, 100, 10):
            ys, hs, xs, ws = square
            crop = img_skel[
                (ys - scale) : (hs + 2 * scale), (xs - scale) : (ws + 2 * scale)
            ]
...
def node_detect(img_tres, predictions):
    ...
    SE_dilate = 20
    kernel_dilate = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (SE_dilate, SE_dilate))
...
def matrix_gen(img_nodes_dilate, predictions, img_norm):
    node_contours, _ = cv2.findContours(
        img_nodes_dilate, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE
    )
    # Device list
    dev = []
    ...
    for i, comp in enumerate(predictions):
        ...
        for j, node in enumerate(node_contours):
            valid, h1, w1 = contourIntersect(
                img_norm, node, square2contour(comp["location"])
            )
            if valid:
